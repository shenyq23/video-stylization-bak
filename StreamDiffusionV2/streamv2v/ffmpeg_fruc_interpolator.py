...
class FFmpegFRUCInterpolator:
    ...
    def interpolate_chunk(
        ...
        # Cap ffmpeg's thread count so it doesn't starve the V4L2 capture thread / DiT consumer.
        # minterpolate defaults to nproc threads (e.g. 28 on a 14-core CPU), which can drag
        # the OpenCV capture loop down to ~8 fps and force V4L2 to drop frames at driver level.
        threads = int(os.environ.get("FRUC_FFMPEG_THREADS", "4"))
        cmd = [
            self.ffmpeg_bin, "-y",
            "-threads", str(threads),
            "-f", "rawvideo",
