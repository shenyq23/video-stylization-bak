...
class LatentWarpVisualizer:
    ...
    def _generate_visuals_loop(
        ...
        for i in range(num_chunks - 1):
            ...
            self.logger.debug(f"latent_n mean: {latent_n_raw.mean():.4f}")
            latent_n = latent_n_raw.transpose(1, 2).contiguous()

            # # 为了给后续VAE的稀疏计算提供cache
            # # ret = self.vae.stream_decode_to_pixel(latent_n, is_full=True)
            # latent_n1_raw = self.vae.stream_encode(chunk_n1_frames.to(self.device, dtype=torch.bfloat16), is_full=True)
            ...
            # # print(f"fused_latent_for_decode mean: {fused_latent_for_decode.mean():.4f}")
            # decoded_fused_video_chunk = self.vae.stream_decode_to_pixel(fused_latent_for_decode, is_full=True)
            ...
            vae_sparse_mask = self.flow_calculator.compute_binary_occlusion_mask(bwd_occ, top_k_percentage=top_k_percentage)[0, 0]
            # Encoder masks
            mask_enc = dilate_mask(vae_sparse_mask, 2)
            masks_enc = downsample_mask(mask_enc, min_res=(2, 2), dilation=2)
            # Decoder masks
            mask_dec = dilate_mask(vae_sparse_mask, 2)
            masks_dec = downsample_mask(mask_dec, min_res=(2, 2), dilation=2)

            latent_n1_vae = self.vae.stream_encode(chunk_n1_frames.to(self.device, dtype=torch.bfloat16), mask=masks_enc, flow=bwd_flow)
            latent_n1_vae = latent_n1_vae.transpose(1, 2).contiguous()
            vae_sparse_video_chunk = self.vae.stream_decode_to_pixel(latent_n1_vae, mask=masks_dec, flow=bwd_flow)
            

            # loss_sparse = torch.mean((latent_n1_vae - latent_n1_repr)**2).sqrt()
            # self.logger.info(f"Chunk {chunk_id}, top_k={top_k_percentage}, Sparse RMSE: {loss_sparse.item():.4f}")
            ...
            for j in range(num_frames_in_chunk):
                ...
                vae_sparse_frame = vae_sparse_video_chunk[:, j]
                ...
                psnr_sparse_latent_vs_target = calculate_psnr(vae_sparse_frame, frame_n1)
                # psnr_full_latent_vs_target = calculate_psnr(decoded_fused_frame, frame_n1)
                if j == 3:
                    self.logger.info(f"  Chunk {chunk_id} Frame {j} Quality Metrics:")
                    # self.logger.info(f"pixel  | vs Target (预测目标): {psnr_pixel_vs_target:.2f} dB")                    self.logger.info(f"latent | vs Target (预测目标): {psnr_latent_vs_target:.2f} dB")
                    # self.logger.info(f"full | vs Target (预测目标): {psnr_full_latent_vs_target:.2f} dB")
                    self.logger.info(f"sparse | vs Target (预测目标): {psnr_sparse_latent_vs_target:.2f} dB")
...
def main():
    ...
    parser.add_argument("--video_path", type=str, default="data/original.mp4", help="Path to the original input video file.")
    ...
    color_fmt = "%(log_color)s%(asctime)s - %(name)s - %(levelname)s - %(message)s%(reset)s"
